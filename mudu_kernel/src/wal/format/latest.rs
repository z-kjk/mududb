...
pub const LOG_FRAME_VERSION_V1: u32 = 1;
pub const LOG_FRAME_VERSION_LATEST: u32 = LOG_FRAME_VERSION_V1;
pub const LOG_FRAME_HEADER_SIZE: usize = 20;
...
impl LogFrameHeader {
    fn new(lsn: LSN, n_part: u32, size: usize) -> Self {
        Self {
            ...
            version: LOG_FRAME_VERSION_LATEST,
            lsn,
            n_part,
            size: size as u32,
        }
    ...
    pub(crate) fn decode(input: &[u8]) -> RS<Self> {
        ...
        let version = u32::from_be_bytes(input[4..8].try_into().unwrap());
        let lsn = u32::from_be_bytes(input[8..12].try_into().unwrap()) as LSN;
        let size = u32::from_be_bytes(input[12..16].try_into().unwrap());
        let n_part = u32::from_be_bytes(input[16..20].try_into().unwrap());
        if magic != LOG_FRAME_MAGIC {
            return Err(m_error!(EC::DecodeErr, "invalid log frame magic"));
        }
        if !(1..=LOG_FRAME_VERSION_LATEST).contains(&version) {
            return Err(m_error!(
                EC::DecodeErr,
                format!("unsupported log frame version {}", version)
            ));
        }
        Ok(Self {
            magic,
